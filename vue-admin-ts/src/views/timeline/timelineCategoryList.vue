<template>
   <div class="app-container">
      ...
      <edit-TimelineCategory ref="editForm" @onSave="handelOnSaved" />
   </div>
</template>
...
import { TimelineCategoryDto } from "@/api/appService";
import EditTimelineCategory from "./components/edit-timelineCategory.vue";

@Component({
   name: "TimelineCategoryList",
   components: { EditTimelineCategory },
   mixins: [enumFilter],
})
export default class TimelineCategoryList extends ElementTableView {
   @Ref() editForm!: EditTimelineCategory;
   tableItems: TimelineCategoryDto[] = [];
   ...
   // 删除
   async handleDelete(index: number, row: TimelineCategoryDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.timelineCategory.delete({ id: row.id }).then((res) => {
            this.$message({
               type: "success",
               message: "删除成功!",
            });
