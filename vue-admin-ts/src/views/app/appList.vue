   ...
   <div class="app-container">
      <div class="my-4 flex flex-col md:flex-row md:items-center space-y-2  md:space-y-0">
         ...
         <div class="flex-1">
            ...
            <button class="btn btn-white ml-2" @click="handleResetSearch"> <i class="el-icon-refresh mr-1" />重置刷新</button>
         </div>
      ...
      <el-table v-loading="table.listLoading" :data="tableItems" element-loading-text="loading..." border stripe fit highlight-current-row @sort-change="sort">
         ...
         <el-table-column label="clientName" prop="clientName" />
         <el-table-column label="clientType" prop="clientType" />
         <el-table-column label="value" prop="value">
            <template slot-scope="scope">
               <div v-for="(value,key) in scope.row.value" :key="key" class="flex justify-between">
                  <div>{{key}}</div>
      ...
      <edit-App ref="editForm" @onSave="handelOnSaved" />
   </div>
</template>
...
import { AppDto } from "@/api/appService";
...
export default class AppList extends ElementTableView {
   @Ref() editForm!: EditApp;
   tableItems: AppDto[] = [];
   ...
   async fetchData(page: number | undefined = undefined) {
      ...
      await api.app.getPublishList().then((res) => {
         console.log(res);
   ...
   // 删除
   async handleDelete(index: number, row: AppDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.app.delete({ id: row.id }).then((res) => {
            this.$message({
               type: "success",
               message: "删除成功!",
            });
