...
@Component
export default class EditUserPrize extends Vue {
  ...
  async onShowChange(value: boolean) {
    if (value) {
      await api.userPrize.getForEdit({ id: this.form!.id }).then(res => {
        this.form = res.data!;
      });
    } else {
  ...
  defaultData: UserPrizeDto = {
    name: undefined,
    id: 0
  };

  show = false;
  form: UserPrizeDto = { ...this.defaultData };
  ...
  async save() {
    ...
    this.dataForm.validate(async (valid: boolean) => {
      if (valid) {
        if (this.form!.id) {
          await api.userPrize.update({ body: this.form });
        } else {
          await api.userPrize.create({ body: this.form });
        }
        this.show = false;
