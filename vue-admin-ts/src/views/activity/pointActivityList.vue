<template>
   <div class="app-container">
      ...
      <edit-PointActivity ref="editForm" @onSave="handelOnSaved" />
   </div>
</template>
...
import { PointActivityDto } from "@/api/appService";
import EditPointActivity from "./components/edit-pointActivity.vue";

@Component({
   name: "PointActivityList",
   components: { EditPointActivity },
   mixins: [enumFilter],
})
export default class PointActivityList extends ElementTableView {
   @Ref() editForm!: EditPointActivity;
   tableItems: PointActivityDto[] = [];
   ...
   // 删除
   async handleDelete(index: number, row: PointActivityDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.pointActivity.delete({ id: row.id }).then((res) => {
            this.$message({
               type: "success",
               message: "删除成功!",
            });
