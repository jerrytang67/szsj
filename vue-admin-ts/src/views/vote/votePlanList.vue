<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="用户上传" prop="isUserUpload" width="80" align="center">
                <template slot-scope="scope">{{ scope.row.isUserUpload ? "是" : "" }}</template>
            </el-table-column>
            <el-table-column label="上传开始时间" prop="uploadStartTime" />
            <el-table-column label="用户上传结束时间" prop="uploadEndTime" />
            <el-table-column label="投票开始时间" prop="voteStartTime" />
            <el-table-column label="投票结束时间" prop="voteEndTime" />
        ...
        />
        <edit-VotePlan ref="editForm" @onSave="handelOnSaved" />
    </div>
</template>
...
import EditVotePlan from "./components/edit-votePlan.vue";
import { VotePlanDto } from "@/api/appService";
...
@Component({
    name: "VotePlanList",
    components: { EditVotePlan },
    mixins: [enumFilter],
})
export default class VotePlanList extends ElementTableView {
    @Ref() editForm!: EditVotePlan;
    tableItems: any[] = [];
    ...
    // 删除
    async handleDelete(index: number, row: VotePlanDto) {
        this.$confirm("你确定删除吗?", "提示").then(async () => {
            await api.votePlan.delete({ id: row.id }).then((res) => {
                this.$message({
                    type: "success",
                    message: "删除成功!",
                });
