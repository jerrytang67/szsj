<template>
   <div class="app-container">
      ...
      <QueryFilter :data.sync="queryForm.status" @change="fetchData" />
      ...
      <el-table
         ...
         @sort-change="sort"
      >
         <el-table-column label="活动ID" prop="votePlanId" width="80" align="center" />
               ...
               <div class="flex flex-col space-y-2">
                  ...
                  <div class="flex items-center justify-arround space-x-1">
                     <div v-for="x in scope.row.form.imageList" :key="x">
                        <img class="h-64 object-contain" data-action="zoom" :src="x" />
                     </div>
                  </div>
               </div>
            </template>
      ...
      />
      <edit-VoteItem ref="editForm" @onSave="handelOnSaved" />
   </div>
</template>
...
import { VoteItemDto } from '@/api/appService';
import EditVoteItem from './components/edit-voteItem.vue'

@Component({
   name: "VoteItemList",
   components: { EditVoteItem },
   mixins: [enumFilter]
})
export default class VoteItemList extends ElementTableView {
   @Ref() editForm!: EditVoteItem;
   tableItems: VoteItemDto[] = [];
   ...
   // 删除
   async handleDelete(index: number, row: VoteItemDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.voteItem.delete({ id: row.id }).then(res => {
            this.$message({
               type: "success",
               message: "删除成功!"
            });
