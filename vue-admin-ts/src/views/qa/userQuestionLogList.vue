<template>
   <div class="app-container">
      ...
      <el-table v-loading="table.listLoading" :data="tableItems" element-loading-text="loading..." border stripe fit highlight-current-row @sort-change="sort">
         ...
         <el-table-column label="planId" prop="planId" width="80" align="center" />
         <el-table-column label="用户" prop="creatorUserId" width="80" align="center" />
         <el-table-column label="奖励积分" prop="points" align="center" />
         <el-table-column label="creationTime" prop="creationTime" />
         <el-table-column label="答对题数" prop="rightCount" />
...
import { UserQuestionLogDto } from "@/api/appService";
...
export default class UserQuestionLogList extends ElementTableView {
   tableItems: UserQuestionLogDto[] = [];
   ...
   // 删除
   async handleDelete(index: number, row: UserQuestionLogDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.userQuestionLog.delete({ id: row.id }).then((res) => {
            this.$message({
               type: "success",
               message: "删除成功!",
            });
