<template>
   <div class="app-container">
      ...
      <el-table v-loading="table.listLoading" :data="tableItems" element-loading-text="loading..." border stripe fit highlight-current-row @sort-change="sort">
         ...
         <el-table-column label="luckDraw" prop="luckDrawId" />
         <!-- <el-table-column label="subTitle" prop="subTitle" /> -->
         <el-table-column label="QuestionCount" prop="questionCount" />
         <el-table-column label="datetimeStart" prop="datetimeStart" />
         <el-table-column label="datetimeEnd" prop="datetimeEnd" />
      ...
      <edit-QaPlan ref="editForm" @onSave="handelOnSaved" />
   </div>
</template>
...
import { QAPlanDto } from "@/api/appService";
import EditQaPlan from "./components/edit-qaPlan.vue";

@Component({
   name: "QAPlanList",
   components: { EditQaPlan },
   mixins: [enumFilter],
})
export default class QaPlanList extends ElementTableView {
   @Ref() editForm!: EditQaPlan;
   tableItems: QAPlanDto[] = [];
   ...
   // 删除
   async handleDelete(index: number, row: QAPlanDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.qaPlan.delete({ id: row.id }).then((res) => {
            this.$message({
               type: "success",
               message: "删除成功!",
            });
