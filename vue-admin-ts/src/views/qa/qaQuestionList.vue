<template>
   <div class="app-container">
      ...
      >
         ...
         <el-table-column label="活动ID" prop="planId" width="80" align="center" />
         <el-table-column label="题目" prop="title" />
         <el-table-column label="答案" prop="answerIndex" width="60" align="center">
            <template slot-scope="scope">{{ String.fromCharCode(scope.row.answerIndex + 65) }}</template>
         </el-table-column>
      ...
      />
      <edit-QaQuestion ref="editForm" @onSave="handelOnSaved" />
   </div>
</template>
...
import { QAQuestionDto } from "@/api/appService";
import EditQaQuestion from "./components/edit-qaQuestion.vue";

@Component({
   name: "QAQuestionList",
   components: { EditQaQuestion },
   mixins: [enumFilter],
})
export default class QaQuestionList extends ElementTableView {
   @Ref() editForm!: EditQaQuestion;
   tableItems: QAQuestionDto[] = [];
   ...
   async fetchData(page: number | undefined = undefined) {
      ...
      if (!this.table.sorting) {
         this.table.sorting = "creationTime desc"
      }

      await api.qaQuestion
         .getAll({
   ...
   // 删除
   async handleDelete(index: number, row: QAQuestionDto) {
      this.$confirm("你确定删除吗?", "提示").then(async () => {
         await api.qaQuestion.delete({ id: row.id }).then((res) => {
            this.$message({
               type: "success",
               message: "删除成功!",
            });
