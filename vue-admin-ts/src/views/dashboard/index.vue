<template>
   <div class="px-8 py-4">
      <div class="mt-4 grid grid-cols-3 gap-4">
         <div class="card zoom-in flex items-center">
            <div class="p-3 rounded-full bg-blue-500 bg-opacity-75">
               <svg class="h-8 w-8 text-white" viewBox="0 0 28 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M18.2 9.08889C18.2 11.5373 16.3196 13.5222 14 13.5222C11.6804 13.5222 9.79999 11.5373 9.79999 9.08889C9.79999 6.64043 11.6804 4.65556 14 4.65556C16.3196 4.65556 18.2 6.64043 18.2 9.08889Z" fill="currentColor" />
                  <path d="M25.2 12.0444C25.2 13.6768 23.9464 15 22.4 15C20.8536 15 19.6 13.6768 19.6 12.0444C19.6 10.4121 20.8536 9.08889 22.4 9.08889C23.9464 9.08889 25.2 10.4121 25.2 12.0444Z" fill="currentColor" />
                  <path d="M19.6 22.3889C19.6 19.1243 17.0927 16.4778 14 16.4778C10.9072 16.4778 8.39999 19.1243 8.39999 22.3889V26.8222H19.6V22.3889Z" fill="currentColor" />
                  <path d="M8.39999 12.0444C8.39999 13.6768 7.14639 15 5.59999 15C4.05359 15 2.79999 13.6768 2.79999 12.0444C2.79999 10.4121 4.05359 9.08889 5.59999 9.08889C7.14639 9.08889 8.39999 10.4121 8.39999 12.0444Z" fill="currentColor" />
                  <path d="M22.4 26.8222V22.3889C22.4 20.8312 22.0195 19.3671 21.351 18.0949C21.6863 18.0039 22.0378 17.9556 22.4 17.9556C24.7197 17.9556 26.6 19.9404 26.6 22.3889V26.8222H22.4Z" fill="currentColor" />
                  <path d="M6.64896 18.0949C5.98058 19.3671 5.59999 20.8312 5.59999 22.3889V26.8222H1.39999V22.3889C1.39999 19.9404 3.2804 17.9556 5.59999 17.9556C5.96219 17.9556 6.31367 18.0039 6.64896 18.0949Z" fill="currentColor" />
               </svg>
            </div>
            <div class="mx-5">
               <h4 class="text-2xl font-semibold text-gray-700">{{dashboardDate.userCount}}</h4>
               <div class="text-gray-500">平台用户数</div>
            </div>
         </div>

         <div class="card zoom-in flex items-center">
            <div class="p-4 rounded-full bg-red-600 bg-opacity-75">
               <svg-icon name="craftsman" width="24" height="24" style="color:white" />
            </div>

            <div class="mx-5">
               <h4 class="text-2xl font-semibold text-gray-700">{{dashboardDate.craftsmanCount}}</h4>
               <div class="text-gray-500">“红色工匠”自荐数</div>
            </div>
         </div>
         <div class="card zoom-in flex items-center">
            <div class="p-4 rounded-full bg-green-600 bg-opacity-75">
               <svg-icon name="craftsman" width="24" height="24" style="color:white" />
            </div>

            <div class="mx-5">
               <h4 class="text-2xl font-semibold text-gray-700">{{dashboardDate.craftsmanRecommendCount}}</h4>
               <div class="text-gray-500">“红色工匠”推荐数</div>
            </div>
         </div>
      </div>
      <div class="mt-8 grid grid-cols-2 gap-4">
         <div>
            <LineChart v-if="chatList1" :smooth="true" lineColor="#4F46E5" areaColor="#C7D2FE" :chart-data="chatList1" :title="['“红色工匠”自荐每日数据']" :y-key="'label'" :x-key="'count'" />
         </div>
         <div>
            <LineChart v-if="chatList2" :smooth="true" lineColor="#FBBF24" areaColor="#FEF3C7" :chart-data="chatList2" :title="['“红色工匠”推荐每日数据']" :y-key="'label'" :x-key="'count'" />
         </div>
      </div>
   </div>

</template>

<script lang="ts">
import { Component, Vue, Prop, Watch, Ref } from "vue-property-decorator";
import api from "@/api";

import LineChart from "./components/LineChart.vue";
import PieChart from "./components/PieChart.vue";
import countTo from "vue-count-to";
import dayjs from "dayjs";

const date = new Date();
const today = dayjs(date).add(-24, "hours").format("YYYY-MM-DD HH:mm:ss");

@Component({ components: { LineChart, PieChart, countTo } })
export default class Index extends Vue {
   dashboardDate: any = {
      userCount: 0,
      craftsmanCount: 0,
      craftsmanRecommendCount: 0,
   };

   async created() {
      await Promise.all([
         api.dashboard.getDashboard().then((res) => {
            this.dashboardDate = res;
         }),
      ]);

      await this.fetchDayData();
   }
   time: number | null = 1;
   chatList1: any = null;
   chatList2: any = null;

   async fetchDayData() {
      let from = "";
      if (this.time === 1) from = today;
      else from = "";
      await api.craftsman.dateAnlayse({ status: 1 }).then((res) => {
         this.chatList1 = res;
      });

      await api.craftsmanRecommend.dateAnlayse({ status: 1 }).then((res) => {
         this.chatList2 = res;
      });
   }
}
</script>