<template>
   <div v-if="!item.meta || !item.meta.hidden" :class="[isCollapse ? 'simple-mode' : 'full-mode', {'first-level': isFirstLevel}]">
      <template v-if="!alwaysShowRootMenu && theOnlyOneChild && !theOnlyOneChild.children">
         <sidebar-item-link v-if="theOnlyOneChild.meta" :to="resolvePath(theOnlyOneChild.path)">
            <el-menu-item :index="resolvePath(theOnlyOneChild.path)" :class="{'submenu-title-noDropdown': isFirstLevel}">
               <svg-icon width="1.5em" height="1.5em" v-if="theOnlyOneChild.meta.icon" :name="theOnlyOneChild.meta.icon" />
               <span v-if="theOnlyOneChild.meta.title" slot="title">
                  ...
                  <div class="flex items-center justify-between w-full">
                     ...
                     <template v-if="theOnlyOneChild.meta.num">
                        <div class="bg-yellow-500 text-white w-6 h-6 text-sm rounded-full flex items-center justify-center ml-4 font-bold">
                           {{theOnlyOneChild.meta.num}}</div>
                     </template>
                  </div>
               </span>
            </el-menu-item>
      ...
      <el-submenu v-else :index="resolvePath(item.path)" popper-append-to-body>
         <template slot="title">
            <svg-icon width="1.5em" height="1.5em" v-if="item.meta && item.meta.icon" :name="item.meta.icon" />
